...
dfall_logic <- dfall %>% filter(dx_ngs_cat=="<30 days" & recent_cat!="Indeterminant" ) %>% 
                            mutate(Recenct_Infection = ifelse(recent_cat == "Recent", 1, 0), 
                                   ngscollectyr = as.factor(ngscollectyr),
                                   ...
                                   owning_jd_region_fsu = as.factor(owning_jd_region_fsu),
                                   incluster = as.factor(incluster),
                                   Recenct_Infection = as.factor(Recenct_Infection)
                                   )
...
dfall_logic <- dfall_logic %>% select(Race, Gender, Age, Year, Region, Cluster, Recenct_Infection)
