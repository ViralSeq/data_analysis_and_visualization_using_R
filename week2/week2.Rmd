...
The aim of this module is to familiarize you with how R works and how to read in and apply basic manipulations on your data. Here, we will be working with similar but slightly shorter version of the Global Burden of Disease dataset than we did for the bar plots.
...
- Choose "upstream/main", merge into your own "Main". After merging, click "PushFetch Origin". 
...
* Restart R (Control+Shift+F10 select it from Session -> Restart R).
...
1. Insert a new code chunk here (there is a `+C Insert` button at the top of this document). Or use shortcut `Control+Alt+I` or `CMD+Option+I`
2. Add `mydata %>% names()` and `mydata %>% head()` to this new chunk and run them. Feel free to copy-paste from the instructions, or if typing in, make sure to use the keyboard shortcut `Control+Shift+M` or `CMD+Shift+M`.
...
Examples will make this clearer, try this code, running it line-by-line (Ctrl+Enter or CMD+Enter):
...
How would you change the above code to only include years greater than 2000 (so not including 2000 itself too)?
...
paste("UNC", "is", "Great", sep = ".")

#If we really like UNC
paste("UNC", "is", "Great", sep = "!")

#If we want to make it one word
paste("UNC", "is", "Great", sep = "") # no separator (still need the brackets)
```
...
Under Standing different type of join.
![img](https://letsdobigdata.files.wordpress.com/2016/03/joins.png)
...
In R, the `summary()` function provides a quick way of summarizing both data or the results of statistical tests.
